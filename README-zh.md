...
OpenMAIC 内置了一个 [OpenClaw](https://github.com/anthropics/openclaw) skill，用来以“逐步确认”的 SOP 方式引导用户完成 clone、启动方式选择、联网搜索方式选择、Provider Key 配置，以及课堂生成。
...
可以在 `~/.openclaw/openclaw.json` 中提供可选默认值：
  ...
  "skills": {
    "entries": {
      "openmaic": {
        "enabled": true,
        "config": {
          "repoDir": "/path/to/OpenMAIC",       // 可选：默认复用的本地仓库路径
          "url": "http://localhost:3000"        // 可选：默认服务地址
        }
      }
    }
  }
...
这些值只是默认值。skill 仍然应该在每个会修改状态的步骤前向用户确认。
...
| 启动 | 让用户在 `pnpm dev`、`pnpm build && pnpm start`、Docker 之间做选择 |
| 搜索 | 询问是否启用 Tavily 驱动的联网搜索 |
| Provider Key | 推荐配置路径，并在写 `.env.local` 或 `server-providers.yml` 前确认 |
| 生成 | 在读取 PDF 或发送生成请求前确认 |

### 使用示例

```
你：「用 OpenMAIC 帮我搭一个本地课堂生成器」
智能体：「我已经找到或可以 clone 这个仓库。你要我复用现有 checkout，还是新 clone 一份？」
```

```
你：「根据这个 PDF 生成课堂」
智能体：「OpenMAIC 服务已经健康。我需要先读取 PDF，再发送生成请求。要继续吗？」
```

这样整个流程会更透明，用户会在每个关键步骤拿到确认感，而不是被一个黑盒插件一口气跑完整套流程。
